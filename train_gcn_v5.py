...
"""
Strong contrastive graph encoder (WITH validation eval + EARLY STOPPING + SCHEDULER)

- CLIP-style contrastive loss with memory queue
- Validation ONLY for model selection (MRR / Recall@k)
- Early stopping based on VAL MRR (NOT loss)
- Cosine LR scheduler
- Saves LAST and BEST checkpoints
...
LOG_CSV = f"losses/log_{version_gnn}.csv"
...
def main():
    ...
    with open(LOG_CSV, "w", newline="") as f:
        csv.writer(f).writerow(
            ["epoch", "loss", "scale", "lr", "val_MRR", "val_R@1", "val_R@5", "patience"]
        )
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, T_max=EPOCHS, eta_min=MIN_LR
    )
    ...
    for ep in range(EPOCHS):
        ...
        if val_mrr > best_mrr + MIN_DELTA:
            best_mrr = val_mrr
            best_epoch = ep + 1
            epochs_no_improve = 0
            ensure_dir(BEST_CKPT_PATH)
            torch.save(
                {"state_dict": model.state_dict(), "best": {"epoch": best_epoch, "val_mrr": best_mrr}},
                BEST_CKPT_PATH,
            )
        else:
            epochs_no_improve += 1
        ...
        print(
            f"Epoch {ep+1:02d}/{EPOCHS} | loss={loss:.4f} | scale={scale:.2f} | "
            f"lr={lr_now:.2e} | val MRR={val_mrr:.4f} | "
            f"patience={epochs_no_improve}/{PATIENCE}"
        )

        with open(LOG_CSV, "a", newline="") as f:
            csv.writer(f).writerow(
                [ep + 1, loss, scale, lr_now,
                 val_scores["MRR"], val_scores["R@1"], val_scores["R@5"],
                 epochs_no_improve]
            )
    ...
    print(f"\nBest val MRR = {best_mrr:.4f} at epoch {best_epoch}")
